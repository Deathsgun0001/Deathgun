...
const mistralSystemPrompt = "You are DEATHGUN, an intelligent bot. Someone mentioned your owner. Respond with helpful and friendly assistance.";
...
const mistral = async (m, Matrix) => {
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Matrix.sendMessage(m.from, { audio: { url: 'https://i.imgur.com/D3zXRsf.mp4' } }, { quoted: m });
            return;
        }
